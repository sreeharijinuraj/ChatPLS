...
class _UploadbrochuresScreenState extends State<UploadbrochuresScreen> {
  ...
  Future<bool> checkFileExists(String fileName) async {
    try {
      final response = await http.get(
        Uri.parse('http://192.168.0.107:5000/check_file?file_name=$fileName'),
      );
  ...
  Future<void> loadUploadedFiles() async {
    try {
      final response = await http.get(
        Uri.parse('http://192.168.0.107:5000/get_uploaded_files'),
      );
  ...
  Future<void> loadUploadedImages() async {
    try {
      final response = await http.get(
        Uri.parse('http://192.168.0.107:5000/get_uploaded_images'),
      );
        ...
        if (data is List) {
          List<Map<String, String>> imageData = data.map((item) {
            return {
              ...
              "url":
                  "http://192.168.0.107:5000/get_uploaded_images/${item["name"]}",
            };
          }).toList();
      ...
      for (var file in result.files) {
        ...
        try {
          ...
          if (['jpg', 'jpeg', 'png'].contains(file.extension!.toLowerCase())) {
            endpoint = 'http://192.168.0.107:5000/upload_image';
          } else if (file.extension!.toLowerCase() == 'csv') {
            endpoint = 'http://192.168.0.107:5000/upload_csv';
          } else {
            endpoint = 'http://192.168.0.107:5000/upload';
          }
              ...
              onPressed: () async {
                ...
                try {
                  ...
                  String endpoint = isImage
                      ? 'http://192.168.0.107:5000/delete_image_file'
                      : 'http://192.168.0.107:5000/delete_file';
